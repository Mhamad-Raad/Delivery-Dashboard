...
A robust . NET-based backend API for the MalDash application - a multi-vendor delivery and ordering platform designed for residential complexes.
...
MalDash is a comprehensive delivery platform that connects vendors with tenants in residential buildings. The system supports:
...
```
MalDash. sln
├── MalDash.API            # Presentation Layer (Controllers, Hubs, Handlers)
├── MalDash.Application    # Application Layer (Services, DTOs, Validators)
├── MalDash.Domain         # Domain Layer (Entities, Enums, Exceptions)
└── MalDash.Infrastructure # Infrastructure Layer (Data Access, Repositories)
```
...
| Layer | Description |
|-------|-------------|
| **MalDash.API** | REST API controllers, SignalR hubs, exception handlers, and middleware configuration |
| **MalDash.Application** | Business logic services, request/response DTOs, FluentValidation validators, and service interfaces |
| **MalDash.Domain** | Core entities, domain enums, constants, and custom exceptions |
| **MalDash. Infrastructure** | Entity Framework Core DbContext, repository implementations, and external service integrations |
...
1. **Clone the repository**
   ```bash
   git clone https://github.com/Renua-Hassan/MalDash-BackEnd.git
   cd MalDash-BackEnd
   ```
...
2. **Configure the database connection**
   ...
   {
     "ConnectionStrings": {
       "DbConnectionString": "Host=localhost;Database=MalDash;Username=your_user;Password=your_password"
     }
   }
   ```
...
### Account Controller (`/MalDashApi/Account`)
...
### Vendor Controller (`/MalDashApi/Vendor`)
...
### Product Controller (`/MalDashApi/Product`)
...
### Order Controller (`/MalDashApi/Order`)
...
### Category Controller (`/MalDashApi/Category`)
...
### Building Controller (`/MalDashApi/Building`)
...
### Notification Controller (`/MalDashApi/Notification`)
